...
def compute_child_environment(state: TreeNode, hamiltonian: TreeNode, avg_children: Sequence[TreeNode], env_root, i: int):
    ...
    env = np.kron(q0ten.conj(), np.kron(hamiltonian.conn, q0ten))
    for j in range(len(state.children)):
        if j == i:
            continue
        env = single_mode_product(avg_children[j].conn.reshape((1, -1)), env, j)
        assert env.shape[j] == 1
    # upstream axis
    env = single_mode_product(env_root.reshape((1, -1)), env, env.ndim - 1)
